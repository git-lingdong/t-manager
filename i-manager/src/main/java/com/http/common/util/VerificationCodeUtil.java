...
public class VerificationCodeUtil {
	...
	public static String createVerificationCode(OutputStream sos, HttpSession session) {
		try {
			creatImage();
			write(sos);
		} catch (IOException e) {
	...
	private static void creatImage() {
		...
		// 设置背景色
		Color bgcolor = getRandColor(130, 200);
		g.setColor(bgcolor);
		...
		VerificationCodeUtil.code = str1;
		for (int i = 0; i < codeCount; i++) {
			String strRand = str1.substring(i, i + 1);
			g.setColor(getRandColor(50, 140));
			// g.drawString(a,x,y);
